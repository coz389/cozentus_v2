	...
	public function blogview($slug)
	{
		...
		if (empty($data['pserv'])) {
			...
			if (!empty($data['pserv']['toolbenefits_json'])) {
				...
				$data['section_2'] = $data['pserv']['toolbenefits_json'];
			}

			if (!empty($data['pserv']['section_3_json'])) {
				$features = json_decode($data['pserv']['section_3_json'], true);
				$sounique = [];
				if (!empty($features))
					foreach ($features as $ft) {
						$sounique[] = ['heading' => $ft['heading'], 'description' => $ft['description']];
					}
				$data['pserv']['section_3_json'] = $sounique;
				$data['section_3'] = $data['pserv']['section_3_json'];
			}

			if (!empty($data['pserv']['section_4_json'])) {
				$features = json_decode($data['pserv']['section_4_json'], true);
				$sounique = [];
				if (!empty($features))
					foreach ($features as $ft) {
						$image = $this->home_model->getimagebyid($ft['image']);
				...
				$data['pserv']['section_4_json'] = $sounique;

				$data['section_4'] = $data['pserv']['section_4_json'];
			}
			*/
