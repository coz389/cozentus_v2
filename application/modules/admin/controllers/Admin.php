...
class Admin extends MX_Controller
{
	...
	if (!empty($focus_json_content_exist ))
		  for ($i = 0; $i <= count($focus_json_content_exist ); $i++) {
			...
			$focus_json_content_dtArr[] = [
				// 'image' => $focus_json_content_exist [$i]['image'], 
				 'heading' => $focus_json_content_exist[$i]['heading'], 'description' => $focus_json_content_exist[$i]['description'], 'ctalink' => $focus_json_content_exist[$i]['ctalink']];
		  }
		...
		for ($i = 0; $i <= count($focus_json); $i++) {
		  if (!empty($focus_json[$i]['heading'])) {
			...
			// 'ctabtn' => $focus_json_content[$i]['ctabtn'], 
			 ...
			 'heading' => $focus_json[$i]['heading'], 'description' => $mdes,'ctalink' => $focus_json[$i]['ctalink']];
		  }
		}
		...
		$tailored_json_content_dtArr[] = [
		  'image' => $tailored_json_content_exist [$i]['image'], 
		   'heading' => $tailored_json_content_exist[$i]['heading'], 'description' => $tailored_json_content_exist[$i]['description'], 'ctalink' => $tailored_json_content_exist[$i]['ctalink']];
		}
		...
		// 'ctabtn' => $tailored_json_content[$i]['ctabtn'], 
		 ...
		 'heading' => $tailored_json[$i]['heading'], 'description' => $mdes,'ctalink' => $tailored_json[$i]['ctalink']];
		}
	 }
		...
		$data = [
			'slug' => $slug,
			'card_heading' => $card_heading, 'card_description' => $card_description, 'card_image' => $card_image,
			'inner_banner_heading' => $inner_banner_heading, 'inner_banner_description' => $inner_banner_description, 'inner_banner_image' => $inner_banner_image,
			'inner_banner_mimage' => $inner_banner_mimage, 'ctabtn' => $ctabtn, 'ctalink' => $ctalink,
			'about_heading' => $about_heading, 'about_short_description' => $about_short_description,
			'advantage_heading' => $advantage_heading, 'advantage_short_description' => $advantage_short_description, 'advantage_image' => $advantage_image, 'advantage_video' => $video,
			'transformation_heading' => $transformation_heading, 'transformation_short_description' => $transformation_short_description, 'transformation_image' => $transformation_image,
	...
	function addupdatesubservice()
	{
		...
		$video = !empty($video) ? urlencode($video) : "";
		$ctalink = !empty($ctalink) ? urlencode($ctalink) : "";
		$data = ['pserv' => $pserv, 'sserv' => $sserv, 'slug' => $slug,
			'card_heading' => $card_heading, 'card_description' => $card_description, 'card_image' => $card_image,  'is_home' => $is_home,
			'inner_banner_heading' => $inner_banner_heading, 'inner_banner_description' => $inner_banner_description, 'inner_banner_image' => $inner_banner_image,
			'inner_banner_mimage' => $inner_banner_mimage, 'ctabtn' => $ctabtn, 'ctalink' => $ctalink,'why_heading ' => $why_heading,
			'about_heading' => $about_heading, 'about_image' => $about_image, 'about_short_description_left' => $about_short_description_left, 'about_short_description_right' => $about_short_description_right,
			'industries_heading' => $industries_heading, 'industries_image' => $industries_image, 'industries_short_description_left' => $industries_short_description_left,
			'advantage_heading' => $advantage_heading, 'advantage_short_description' => $advantage_short_description, 'advantage_image' => $advantage_image, 'advantage_video' => $video,
			'buisness_heading' => $buisness_heading, 'buisness_json' => json_encode($buisness_content_dtArr), 'toolbenefits_heading' => $toolbenefits_heading, 'toolbenefits_json' => json_encode($toolbenefits_content_dtArr), 'heading_faq' => $heading_faq, 
			'card_json_faq' => json_encode($faq_dtArr),'believe_heading' => $believe_heading, 'believe_json' => json_encode($believe_pointArr),'mbanners'=>json_encode($mbanners_dtArr), 'info_heading' => $info_heading, 'info_image' => $info_image, 
			'page_header' => $page_header,'tags' => $tags,'cardsecop'=>json_encode($cardop_dtArr),'cardsecop_heading' => $heading_cardop,'main_cta_btn'=>$mainctabtn,'main_cta_link'=>$mainctalink,'main_cta_pdf'=>$mainctapdf];
...
function addgalleryvideo()
{
	...
	function insertupdatehomesig()
	{
		...
		$ctabtn = $this->input->post('ctabtn');
		$id = $this->input->post('id');
		$data = array('whytitle' => htmlentities($whytitle),'title' => htmlentities($title),'significant_1' => htmlentities($significant_1),'significant_2' => htmlentities($significant_2),'significant_3' => htmlentities($significant_3),'significant_4' => htmlentities($significant_4),'significant_5' => htmlentities($significant_5),'significant_6' => htmlentities($significant_6),'image' => $image, 'mob_image' => $mimage, 'short_description' => $shortdescription, 'orderby' => $orderby, 'cta_btn' => $ctabtn, 'cta_link' => urlencode($ctalink));
		if (!empty($id)) {
