function formVal() {
	...
	$.validator.addMethod("eml", function (value, element) {
		...
		return (
			...
			(match && disallowedDomains.indexOf(match[1].toLowerCase()) === -1)
		);
	});
	$form.each(function () {
		var Mainform = $(this);
				...
				$.ajax({
					...
					success: function (res) {
						...
						if (Mainform.data("download")) {
							downloadURI(
								Mainform.data("download"),
								Mainform.data("file-name")
							);
						}
						if (res.status) {
...
function renderSuccess(btn, res, type) {
	var st = `
    <div class="${type == "success" ? "success-bar" : "error-bar"} state-bar">${
		res.msg
	}</div>
    `;
